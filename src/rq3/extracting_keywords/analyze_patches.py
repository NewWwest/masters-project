...
cpus = 20
data_location = r'results\patch_data'
batch_size = 1024
positive_directory = r'results\samples\positive'
background_directory = r'results\samples\background'
token_ranking_file = r'tf-idf.csv'
...
def rank_tokens():
    posi_files = _enumerate_files(positive_directory)
    positive_tokens = []
    ...
    back_files = _enumerate_files(background_directory)
    background_tokens = []
    ...
    ratio = dict.fromkeys(all_tokens, 0)
    zero_safe_guard = 1 / len(all_tokens)
    for x in tf_positive:
        # ratio[x] = (tf_positive[x] * df_positive[x]) / (tf_background[x] * df_background[x] + zero_safe_guard)
        # ratio[x] = (df_positive[x]) / (df_background[x] + zero_safe_guard)
        ratio[x] = tf_positive[x] / (df_background[x] + zero_safe_guard)

    ratio_sorted = sorted(ratio.items(), key=lambda item: -item[1])
    csv_exporatble = []
...
def mine_tokens():
    data_files = _enumerate_files(data_location)
    with Pool(cpus) as p:
